<div class="card" id="image_layout{{ !empty($id) ? $id : '' }}">
  <div class="card-body">
    ...
    <div class="mt-3">
      <p class="m-0" id="extension_notice"></p>
      <p class="m-0" id="width_height_multiple_notice"></p>
    </div>

    <div class="mt-3" id="image_column">
    </div>
  </div>
</div>
...
@push('script')
  <script>
    ...
    function on_change_multiple_width_height(){
      $('#width_height_multiple_notice').html('Image must be in ' + max_width + 'x' + max_height + " resolution")
    }

    async function on_change_multiple_image(){
      arr_response = []

      var str = ""
      str += `<div class="row">`
      for(let x in arr_url_image){
        str += `<div class="col-6 col-lg-3 mt-3">`
          str += `<div class="position-relative ml-1">`
            if(arr_name_display[x] == null){
              str += `<button class="btn btn-danger position-absolute" style="right: 0" onclick="on_remove_clicked(${x})"><i class="fa-solid fa-trash m-0"></i></button>`
              ...
              <div class="d-flex justify-content-between align-items-center">
                ...
                <button class="btn btn-danger" onclick="on_remove_clicked(${x})"><i class="fa-solid fa-trash m-0"></i></button>
              </div>
              `
            }
          str += `</div>`
      ...
      str += `</div>`
      $('#image_column').html(str)

      onDoneGetImage()
    }

    async function onDoneGetImage(){
      // if(counter_image == arr_response.length){
        // console.log(JSON.stringify(arr_response))
        $('#arr_image{{ !empty($id) ? $id : '' }}').val(JSON.stringify(arr_response))
      // }
    }

    function on_remove_clicked(index){
