...
<script type="text/javascript">
  ...
  $(document).ready(function () {
    jobs_datatable = $('#list_jobs_datatable').DataTable({
      ...
      "ajax" : {
        url : "{{ url('api/jobs?jobs_recommendation_id='.$jobs_recommendation->id) }}",
        type : "GET",
        dataType : "json",
        headers : {
      ...
      "columnDefs" : [
        {
          ...
          render: function(data, type, row, meta) {
            ...
            str += '<div style="width: 150px">'
              str += `<a class="btn btn-primary" onclick="save_current_page('{{ __('jobs_recommendation.detail') }}')" href="{{ url('/jobs/detail') }}?id=${row.id}&from=jobs_recommendation")>{{ __('general.detail') }}</a>`
            str += '</div>'
            return str
          },
        },
