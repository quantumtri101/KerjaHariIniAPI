...
<script type="text/javascript">
  ...
  $(document).ready(function () {
    jobs_datatable = $('#list_jobs_datatable').DataTable({
      ...
      "ajax" : {
        url : "{{ url('api/jobs?worker_id='.$customer_oncall->id) }}",
        type : "GET",
        dataType : "json",
        headers : {
      ...
      "columnDefs" : [
        {
          ...
          render: function(data, type, row, meta) {
            ...
            str += '<div style="width: 150px">'
              str += `<a class="btn btn-primary" onclick="save_current_page('{{ __('customer_oncall.detail') }}')" href="{{ url('/jobs/detail') }}?id=${row.id}&from=customer_oncall")>{{ __('general.detail') }}</a>`
            str += '</div>'
            return str
          },
        },
