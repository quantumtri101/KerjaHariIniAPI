...
<script type="text/javascript">
  ...
  $(document).ready(function () {
    jobs_recommendation_datatable = $('#list_jobs_recommendation_datatable').DataTable({
      ...
      "ajax" : {
        url : "{{ url('api/jobs/recommendation?user_id='.$customer_oncall->id) }}",
        type : "GET",
        dataType : "json",
        headers : {
      ...
      "columnDefs" : [
        {
          ...
          render: function(data, type, row, meta) {
            ...
            str += '<div style="width: 150px">'
              str += `<a class="btn btn-primary" onclick="save_current_page('{{ __('customer_oncall.detail') }}')" href="{{ url('/jobs-recommendation/detail') }}?id=${row.id}")>{{ __('general.detail') }}</a>`
            str += '</div>'
            return str
          },
        },
