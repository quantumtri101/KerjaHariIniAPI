<div class="row">
  ...
  <div class="col-12 d-flex mt-3">
    @if($jobs_shift->end_date->add(2, 'hours') >= \Carbon\Carbon::now() && Auth::user()->type->name == "RO" && $allow_manual_check_log)
    <button data-toggle="modal" data-target="#check_log_manual" class="btn btn-primary mr-3">{{ __('general.manual_check_log') }}</button>
    @endif

    <a class="btn btn-primary" target="_blank" href="{{ url('/jobs/print-qr?id='.$jobs_shift->jobs->id) }}">{{ __('general.print_qr') }}</a>
  </div>
</div>