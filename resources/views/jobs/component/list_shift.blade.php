    ...
    <table class="table w-100" id="list_shift_datatable">
      <thead>
        <tr>
          ...
          <th class="border-b-2 dark:border-dark-5 whitespace-nowrap">{{ __('general.start_date') }}</th>
          <th class="border-b-2 dark:border-dark-5 whitespace-nowrap">{{ __('general.end_date') }}</th>
...
<script type="text/javascript">
  ...
  $(document).ready(function () {
    user_datatable = $('#list_shift_datatable').DataTable({
      ...
      "ajax" : {
        url : "{{ url('api/jobs/shift') }}?jobs_id={{ $jobs->id }}",
        type : "GET",
        dataType : "json",
        headers : {
