          ...
          render: function(data, type, row, meta) {
            ...
            str += '<div style="">'
              ...
              // if(row.is_approve == 1 && row.status == "open")
                str += `<a class="btn btn-primary mr-1" target="_blank" href="{{ url('/jobs/export/pdf') }}?id=${row.id}">
                    ...
                    if(row.is_live_app == 1)
                      ...
                      str += `
                        <form method="post" class="jobsForm" key="${row.id}" action="{{ url('/jobs/change-live') }}" class="d-inline-block ml-1">
                          @csrf
                          <input type="hidden" name="jobs_id" value="${row.id}"/>
                          <input type="hidden" name="publish_start_date" id="publish_start_date_${row.id}" class="publish_start_date" key="${row.id}"/>
                          <input type="hidden" name="publish_end_date" id="publish_end_date_${row.id}" class="publish_end_date" key="${row.id}"/>
                        </form>
