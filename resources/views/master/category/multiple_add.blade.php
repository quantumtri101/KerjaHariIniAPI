...
<form method="post" class="mt-3" action="{{ url('/master/category/multiple') }}" enctype="multipart/form-data">
  @csrf
  <input type="hidden" name="id" value="{{ Request::get('id') }}"/>

  <div class="card mt-3">
    <div class="card-body">
      <input type="hidden" id="arr_category" name="arr_category"/>
      <div class="row">
        <div class="col-12 col-lg-4">
          @include('master.category.component.multiple.add')
        </div>
        <div class="col-12 col-lg-8">
          @include('master.category.component.multiple.table')
        </div>
      </div>
    </div>
  </div>
      ...
      $(document).ready(() => {
        $('#submit').click((e) => {
          if(arr_category.length == 0){
            e.preventDefault()
            notify_user('{{ __("general.list_category_empty") }}')
          }
          else
            back_page(false)
        })
