...
@section('content')
  <h5 class="m-0">{{ __('privacy.edit') }}</h5>

  <div class="my-3">
    <form method="post" action="{{ url('/master/privacy') }}" enctype="multipart/form-data">
      @csrf
      @method('PUT')
      ...
      <div class="card" id="privacy">
        <div class="card-body">
          <div class="form-group d-none">
            ...
            <input type="text" name="title" class="form-control" value="{{ !empty($privacy) ? $privacy->title : '' }}"/>
          </div>
  ...
  @push('script')
    <script>
      $(document).ready(() => {
        ...
        $("#summernote").summernote("code", `{!! $privacy->content !!}`)
      })
    </script>
  @endpush
@endsection
