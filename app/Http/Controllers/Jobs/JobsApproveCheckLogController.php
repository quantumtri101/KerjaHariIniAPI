...
class JobsApproveCheckLogController extends BaseController{
  private $arr_header = [
    [
      "id" => "name",
      "column" => "jobs_approve_check_log.name",
      "name" => "general.name",
      "data_type" => "string",
    ],
  ];
  ...
  public function index(Request $request){
    ...
    //   $this->relationship_helper->jobs_approve_check_log($data, $key);
  ...
  private function manage_where($request, $type = 'index'){
    ...
    $arr = JobsApproveCheckLog::select($jobs_approve_check_log_model->get_table_name().'.*', $user_model->get_table_name().'.name as user_name',)
      ->join($user_model->get_table_name(), $jobs_approve_check_log_model->get_table_name().'.user_id', '=', $user_model->get_table_name().'.id');

    if(!empty($request->id))
      $arr = $arr->where('id', '=', $request->id);

    if(!empty($request->jobs_id))
      $arr = $arr->where($jobs_approve_check_log_model->get_table_name().'.jobs1_id', '=', $request->jobs_id);

    if(!empty($request->status_approve))
      $arr = $arr->where($jobs_approve_check_log_model->get_table_name().'.status_approve', '=', $request->status_approve);
