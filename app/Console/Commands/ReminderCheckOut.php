...
class ReminderCheckOut extends Command
{
  /**
  ...
  * @var string
  */
  protected $signature = 'reminder:check_out';
  ...
  */
  public function handle()
  {
    ...
    $arr_shift = JobsShift::select($jobs_shift_model->get_table_name().'.*')
      ...
      ->where($jobs_shift_model->get_table_name().'.end_date', '<', Carbon::now()->addHours(1))
    ...
    foreach($arr_shift as $shift){
      ...
      foreach($arr_application as $application)
        $this->communication_helper->send_push_notif($user, 'Reminder Check Out', 'Mengingatkan, untuk melakukan Check Out, untuk Pekerjaan '.$application->jobs->name, ["id" => $application->jobs->id, 'type' => "jobs_reminder"]);
    }
  }
}
